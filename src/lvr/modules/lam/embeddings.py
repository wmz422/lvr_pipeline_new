...
def exists(val) -> bool:
    """None 检查的小工具，保留自 RoPE 实现。"""
    return val is not None


def default(val, d) -> Tensor:
    """如果 val 为 None 则返回默认值 d。"""
    return val if exists(val) else d


def broadcat(tensors, dim=-1) -> Tensor:
    """先 broadcast 多个张量，再按指定维度拼接。"""
...
def apply_rotary_emb(freqs, t, start_index=0, scale=1.0, seq_dim=-2):
    """把 rotary frequency 应用到 query/key 的一段 hidden 维度上。"""
    dtype = t.dtype
...
class RotaryEmbedding(Module):
    """LAM temporal attention 使用的 RoPE 实现。"""
